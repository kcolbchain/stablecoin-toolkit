...
async function main() {
  ...
  console.log("Deploying UAE stablecoin with account:", deployer.address);

  const geoConfig = JSON.parse(
    fs.readFileSync(path.resolve(__dirname, "../config/geographies/uae.json"), "utf8")
  );

  const Stablecoin = await ethers.getContractFactory("Stablecoin");
  const stablecoin = await Stablecoin.deploy("UAE Dirham Stablecoin", "AEDX", deployer.address);
  ...
  console.log("UAE deployment complete");
}
