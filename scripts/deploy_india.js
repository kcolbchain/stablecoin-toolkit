...
async function main() {
  ...
  console.log("Deploying India stablecoin with account:", deployer.address);

  const geoConfig = JSON.parse(
    fs.readFileSync(path.resolve(__dirname, "../config/geographies/india.json"), "utf8")
  );

  const Stablecoin = await ethers.getContractFactory("Stablecoin");
  const stablecoin = await Stablecoin.deploy("India Rupee Stablecoin", "INDR", deployer.address);
  ...
  console.log("Minter authorized on stablecoin");
  console.log("India deployment complete");
}
